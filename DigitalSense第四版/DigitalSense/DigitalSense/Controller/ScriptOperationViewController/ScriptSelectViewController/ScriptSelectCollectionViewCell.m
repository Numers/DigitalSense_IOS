...
-(void)setFruitSmell:(Fruit *)fruit WithScriptCommand:(ScriptCommand *)scriptCommand;
{
    ...
    if (self.smellView) {
        ...
        [self.smellView setFrame:CGRectMake(self.smellView.frame.origin.x, self.frame.size.height * (1 - scriptCommand.power), self.smellView.frame.size.width, self.frame.size.height * scriptCommand.power)];
    }else{
